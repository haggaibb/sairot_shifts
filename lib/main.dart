...
// Sensitive credentials removed - use environment variables or secure storage
...
class Controller extends GetxController {
  ...
  RxList<Instructor> allInstructorsList = RxList<Instructor>();
  ...
  loadTomorrowDayInstructors() async {
    ...
    if (today.day >= endDate.value.day || today.day < startDate.value.day - 1)
  ...
  Future<Map<String, dynamic>> loadAdminEvent(String eventId) async {
    final eventDoc =
        FirebaseFirestore.instance.collection('Events').doc(eventId);
    final eventSnapshot = await eventDoc.get();
    final eventMetadata = eventSnapshot.data() ?? {};

    // Get instructors subcollection
    final instructorsSnap = await eventDoc.collection('instructors').get();
    final List<Instructor> instructors = instructorsSnap.docs
        .map((doc) => Instructor.fromMap(doc.data(), doc.id))
        .toList();

    // Get event_days from subcollection, using the `date` field inside each doc
    final daysSnap = await eventDoc.collection('event_days').get();
    final List<DateTime> eventDays = daysSnap.docs
        .map((doc) {
          final data = doc.data();
          final Timestamp? date = data['date'];
          return date?.toDate();
        })
        .whereType<DateTime>() // removes nulls safely
        .toList();

    // Parse timestamps safely
    final Timestamp? timestampStart = eventMetadata['start_date'];
    final Timestamp? timestampEnd = eventMetadata['end_date'];

    // Build final event map
    final adminEvent = {
      'event_name': eventMetadata['event_name'],
      'start_date': timestampStart?.toDate(),
      'end_date': timestampEnd?.toDate(),
      'instructors_per_day': eventMetadata['instructors_per_day'],
      'event_instructors': instructors.map((i) => i.toMap()).toList(),
      'event_days': eventDays,
    };
    return adminEvent;
  }

  deleteEvent(String eventName) async {
    CollectionReference eventsListRef = db.collection('Events');
    await eventsListRef.doc(eventName).delete();
  }
  ...
  loadEventInstructors() async {
    ...
    eventInstructorsList.forEach((element) {
      ...
      eventInstructors.add(Instructor.fromMap(instructorData, element.id));
    ...
    final snapshot = await db.collection('Instructors').get();
    allInstructorsList.assignAll(
      snapshot.docs
          .map((doc) => Instructor.fromMap(doc.data(), doc.id))
          .toList()
  ...
  String getInstructorFullName(String id) {
    Instructor? foundInstructor = eventInstructors
        .where((i) => i.armyId == id)
        .cast<Instructor?>()
        .firstOrNull; // requires Dart 3.0+
    if (foundInstructor?.firstName != null) {
      return '${foundInstructor?.firstName}  ${foundInstructor?.lastName}';
    } else {
      return 'N/A';
    }
  }
  ...
  updateCheckedIn(selectedDay) async {
    String dateKey =
        "${selectedDay.day.toString().padLeft(2, '0')}-${selectedDay.month.toString().padLeft(2, '0')}-${selectedDay.year.toString()}";
    db
        .collection('Events/$eventName/days')
        .doc(dateKey)
    ...
    for (List day in instructorsPerDay) {
      for (var instructor in day) {
        ...
        if (instructorIndex > -1) {
          eventInstructors[instructorIndex].assignDays =
              eventInstructors[instructorIndex].assignDays + 1;
        }
      }
    }
  }
  ...
  saveEventInstructorsReportCSV() async {
    ...
    await FileSaver.instance.saveFile(
      name:
          'Event Instructors Report ${DateTime.now().toIso8601String()}', // you can give the CSV file name here.
      bytes: bytes,
      ext: 'csv',
  ...
  saveTomorrowInstructorsCSV(
      String title, List<TextEditingController> groups) async {
    List<List<dynamic>> rows = [];
    ...
    await FileSaver.instance.saveFile(
      name:
          '$title  ${DateTime.now().toIso8601String()}', // you can give the CSV file name here.
      bytes: bytes,
      ext: 'csv',
      mimeType: MimeType.csv,
    );
    ...
    return Column(
      children: [
        ...
        Center(
            child: Container(
                width: 350,
                child: Image.network('assets/images/logo_round.png'))),
        Center(
            child: Container(
                child: const Text(
          'טוען נתונים',
